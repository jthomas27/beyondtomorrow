...
| `RESEND_API_KEY` | Resend newsletter API key (`re_...`) — sends post emails to Ghost members | ✅ |
...
for k in ('GITHUB_TOKEN', 'DATABASE_URL', 'GHOST_URL', 'GHOST_ADMIN_KEY', 'EMAIL_HOST', 'EMAIL_PORT', 'EMAIL_USER', 'EMAIL_PASS', 'RESEND_API_KEY'):
    if k in v:
        print(f'{k}={v[k]}')
"
```
